...
def conv2d(x, input_channel, channel, k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02, sn=False, padding="VALID", bias=True, name='conv2d'):
	with tf.variable_scope(name):
		if input_channel == 0:
			w = tf.get_variable('weights', [k_h, k_w, x.get_shape()[-1], channel], initializer=tf.truncated_normal_initializer(stddev=stddev))
		else:
			w = tf.get_variable('weights', [k_h, k_w, input_channel, channel], initializer=tf.truncated_normal_initializer(stddev=stddev))
		if sn:
