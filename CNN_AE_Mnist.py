...
class AutoEncoder(BasicTrainFramework):
    ...
    def build_placeholder(self):
        self.source = tf.placeholder(shape=(self.batch_size, 28, 28, 1), dtype=tf.float32)
        self.target = tf.placeholder(shape=(self.batch_size, 28, 28, 1), dtype=tf.float32)
        # print('source shape is: ', array(range(len(self.source.shape))))
    def build_network(self):
        self.embedding = self.encoder(self.source, is_training=True, reuse=False)
        self.pred = self.decoder(self.embedding, is_training=True, reuse=False)
        self.pred_test = self.decoder(self.embedding, is_training=False, reuse=True)
    ...
    def hist(self, epoch):
        # real = self.sample_data["data"]
        real,label = mnist.test.next_batch(64)
        # real
        real = real.reshape(64,28,28,1)
        pr, _ = np.histogram(real, bins=np.linspace(0., 1, 100), density=True)
        plt.plot(np.linspace(0., 1, len(pr)), pr, label='real', color='g', linewidth=2)
        # pred
        emb, fake = [], []
        for _ in range(1):
            e, f = self.sess.run([self.embedding, self.pred_test], feed_dict={self.source:real})
            emb.append(e)
            fake.append(f)
        pe, _ = np.histogram(np.concatenate(emb), bins=np.linspace(0., 1, 100), density=True)
        plt.plot(np.linspace(0.,1,len(pe)), pe, label='embedding', color='b', linewidth=2)
    ...
    def train(self, epoches=1):
        # batches_per_epoch = self.data.train_num // self.batch_size
        batches_per_epoch = int(mnist.train.num_examples / 64)

        for epoch in range(epoches):
            for idx in range(batches_per_epoch):
                cnt = epoch * batches_per_epoch + idx

                data, label = mnist.train.next_batch(64)
                data = data.reshape(64,28,28,1)
                # data = self.data(self.batch_size, var_list=["data",])

                feed_dict = {self.source:data, self.target:data}
...
def AE():
    ...
    for i in range(100):
        ...
        emb = ae.sess.run(ae.embedding, feed_dict={ae.source:data})
        embs.append(emb)
        labels.append(np.argmax(label, 1).reshape((ae.batch_size,)))
